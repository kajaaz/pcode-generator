...
fn main() -> io::Result<()> {
    ...
    // Adjust the regex to capture the CALLOTHER operation along with all arguments
    let call_other_regex = Regex::new(r"CALLOTHER \(const,0x([0-9a-fA-F]+),([^\)]+)\)").unwrap();
    ...
    for line in reader.lines().filter_map(Result::ok) {
        if let Some(caps) = call_other_regex.captures(&line) {
            ...
            if let Some(op_name) = callother_db.get(&index) {
                operations_used.insert((index, op_name.clone(), args_str.to_string()));
                writeln!(output_file, "CALLOTHER '{}' invoked with Index: 0x{:x}, Arguments: {}", op_name, index, args_str)?;
            }
        }
    }

    writeln!(output_file, "Used CALLOTHER operations:")?;
    for (index, name, args) in &operations_used {
        writeln!(output_file, "Index: 0x{:x}, Name: {}, Arguments: {}", index, name, args)?;
    }
